...
private var playerPos = Vector3.zero;
...
function FixedUpdate() {
	...
	if(isAlive && myTile != 9999){
		...
		if (IsGrounded() && moveDirection != Vector3.zero)
		{
			...
			var rotSpeed = 1000 * Time.deltaTime;
			transform.rotation = Quaternion.RotateTowards( transform.rotation, targetRot, rotSpeed );
			transform.rotation.z = 0;
			transform.rotation.x = 0;
			//moveDirection = transform.forward * Time.deltaTime * (moveSpeed * 20);
			moveDirection.y = 0;
		}else{
			var xzMove = movement;
			xzMove.y = 0;
		...
		var tempMain = GameObject.FindObjectOfType(main);
		var tempTile = tempMain.tileArray[lastTile];
		var tempPos = tempTile.transform.position;
		GameObject.Find("Player(Clone)").transform.position = tempPos;
	}
}
